    ...
    sidebarPanel(
      ...
      div(id = "controls",
          ...
          tags$details(
            ...
            selectInput("anova_type", "ANOVA type (sum of squares)",
                        choices = c("Type II"  = "2",
                                    "Type III" = "3"),
                        selected = "3"),
            ...
            helpText("Type III main-effect tests depend on the contrast coding ",
                     "when interactions are present; this app applies sum-to-zero ",
      ...
      tabsetPanel(
        ...
        tabPanel("Explore (raw data)",
                 ...
                 helpText("Counts per combination of the selected categorical ",
                          "fixed effects. Empty cells (shaded) are what break ",
                          "EMMeans / cld \u2014 check for them before fitting."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    # deletion is caught here rather than producing a cryptic lmer error.
    ...
    # conventional treatment coding (familiar summary() coefficients).
    ...
    fit <- tryCatch(
      withCallingHandlers(
        lmerTest::lmer(fml, data = model_df, REML = use_reml,
                       na.action = na.omit, contrasts = contr_arg),
        warning = function(w) {
  ...
  output$fit_table <- DT::renderDataTable({
    ...
    # deviance() is deprecated for REML fits; report the REML criterion there.
    is_reml  <- isTRUE(rv$fit$reml)
    dev_name <- if (is_reml) "REML_criterion" else "deviance"
    dev_val  <- tryCatch(
      if (is_reml) as.numeric(lme4::REMLcrit(mod)) else deviance(mod, REML = FALSE),
      error = function(e) NA_real_)
    stats <- list(
      AIC = AIC(mod), BIC = BIC(mod),
      logLik = as.numeric(logLik(mod)))
    stats[[dev_name]] <- dev_val
    stats <- c(stats, list(n_obs_used = n_used, n_obs_total = n_total,
  ...
  output$ranef_plot <- renderPlot({
    ...
    if (n == 1) {
      ...
      # stack one panel per grouping factor on a single device
      for (i in seq_len(n))
  ...
  output$anova_table <- renderPrint({
    req(rv$fit)
    type_roman <- if (rv$fit$atype == "3") "III" else "II"
    warns <- character(0)
    run_anova <- function(ddf) withCallingHandlers(
      anova(rv$fit$mod, type = type_roman, ddf = ddf),
      warning = function(w) {
    ...
    if (bad_p || bad_ddf || any(grepl("NaN", warns, ignore.case = TRUE)))
      cat("\n\nNote: one or more p-values could not be computed (NaN). The denominator\n",
          "degrees of freedom for those terms can't be estimated \u2014 this happens when\n",
          "the fit is singular / over-parameterized (see the fit warnings on the model\n",
          "run). Try an intercept-only random structure (drop the random slope), remove\n",
          "a sparse factor (Explore > design balance), or center & scale continuous\n",
  ...
  output$compare_lrt <- renderPrint({
    ...
    if (na_used != nb_used)
      cat(sprintf("WARNING: the models were fit on different numbers of rows (A: %d, B: %d),\n",
                  na_used, nb_used),
          ...
          sep = "")
    if (random_differ)
      cat("NOTE: the random-effects structures differ. Testing whether a variance\n",
          "component is zero places the null on the boundary of the parameter space,\n",
          "so the ordinary chi-square LRT is conservative (the true p-value is smaller;\n",
          "a common correction halves it). Interpret such tests with care.\n\n", sep = "")
    if (fixed_differ && !random_differ)
      cat("NOTE: the models differ in fixed effects. anova() refits both with ML\n",
